...
async def set_var(client: Client, message: Message):
    if len(message.command) < 3:
        return await edit_or_reply(
            message, f"<b>Penggunaan:</b> {CMD_HANDLER}setvar [Var Name] [Var Value]"
        )
...
async def varget_(client: Client, message: Message):
    if len(message.command) != 2:
        return await message.reply(
            f"<b>Penggunaan:</b> {CMD_HANDLER}getvar [Var Name]"
        )
    Cilik = await message.reply("💈 `Memproses!`")
    ...
    if await is_heroku():
        ...
        if check_var in heroku_config:
            ...
            return await Cilik.edit(f"Tidak dapat menemukan var {check_var}")
    else:
        path = dotenv.find_dotenv("config.env")
        if not path:
            return await Cilik.edit(".env file not found.")
        ...
        if not output:
            await Cilik.edit(f"Tidak dapat menemukan var {check_var}")
        else:
            return await Cilik.edit(f"<b>{check_var}:</b> <code>{str(output)}</code>")


@Client.on_message(filters.command("delvar", [".", "-", "^", "!", "?"]) & filters.me)
async def vardel_(client: Client, message: Message):
    if len(message.command) != 2:
        return await message.edit(f"<b>Penggunaan:</b> {CMD_HANDLER}delvar [Var Name]")
    Cilik = await message.reply("💈 `Memproses!`")
    check_var = message.text.split(None, 2)[1]
    if await is_heroku():
        if HAPP is None:
...
async def usage_heroku(client: Client, message: Message):
    ...
    if await is_heroku():
        ...
        return await message.reply("Hanya untuk Aplikasi Heroku")
    dyno = await message.reply("🔮 `Memeriksa Dyno anda. Harap tunggu!`")
...
╔════════════════════╗
 ⚡️ **Penggunaan Dyno Kamu** :
      ►`{AppHours}` **Jam**  `0` **Menit |**  [`{AppPercentage}` **%**]\n
 ⚡️ **Sisa kuota dyno bulan ini** :
      ►  `{hours}` **Jam**  `0` **Menit |**  [`{percentage}` **%**]
╚════════════════════╝
🎰 **Sisa Dyno Heroku** `30` **Hari Lagi**"""
